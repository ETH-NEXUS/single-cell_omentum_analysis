...
vst_out$y[is.na(vst_out$y)] = 0
y_smooth = sctransform::smooth_via_pca(vst_out$y, do_plot = TRUE)
#y_smooth = sctransform::smooth_via_pca(vst_out$y, do_plot = FALSE, max_pc = 80)
...
print(number_genes)
#length(gene_desc[gene_desc$residual_variance > min_var,]$residual_variance)
...
set.seed(0)
# g = scran::buildSNNGraph(tmp, k=30, d=50, pc.approx=T)
