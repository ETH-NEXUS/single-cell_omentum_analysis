...
# Filter cells according to hard threshold of NODG; too many cells, doublet special
max_threshold_NODG = 3200
print("max_threshold_NODG:")
print(max_threshold_NODG)
higher_than_max_threshold <- NODG > max_threshold_NODG
high_abs_NODG <- which(higher_than_max_threshold)
# print out numbers of filtered cells
perc_high_threshold_NODG <- signif((length(high_abs_NODG) / length(input_cell_names)) * 100, digits = 2)
print("Total number of cells:")
print(length(input_cell_names))
print("Cells filtered because NODG is higher than maximum threshold:")
print(length(high_abs_NODG))
print("Percentage of cells filtered because NODG is higher than maximum threshold:")
print(paste(perc_high_threshold_NODG, "%"))

### Apply filters combined
# Merge all filtered cells:
filtered_cells <- unique(c(low_NODG, high_MT, high_abs_MT, low_abs_NODG,high_abs_NODG))
# get cell barcodes of cell that will be filtered out
names_filtered_cells <- unique(names(c(low_NODG, high_MT, high_abs_MT, low_abs_NODG, high_abs_NODG)))
...
perc_filtered_out <- signif((length(unique(c(low_NODG, high_MT, high_abs_MT, low_abs_NODG, high_abs_NODG))) / length(input_cell_names)) * 100, digits = 2)
...
col[higher_than_max_threshold] <- "cyan"
...
abline(v = log2(max_threshold_NODG), col = "cyan")
legend("topright", legend = c("fraction_MT_too_high", "NODG_too_low_too_high", "filtered_both_criteria", "no genes expressed after gene filtering", "dot size corresponds to library size", "absolute threshold fractionMT", "absolute threshold NODG", "doublets"),
