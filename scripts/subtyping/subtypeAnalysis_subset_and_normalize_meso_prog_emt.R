...
take_meso = c(1, 4, 13, 18)
take_progenitor= c(2, 8)
take_emt = c(24)

all_subset <- combined[, combined$umap_cohort_cl %in% c(take_meso, take_progenitor, take_emt)]
all_subset$subtype <- ""
all_subset$subtype[all_subset$umap_cohort_cl %in% take_meso] <- "Mesothelial.cells"
all_subset$subtype[all_subset$umap_cohort_cl %in% take_progenitor] <- "Progenitor.cells"
all_subset$subtype[all_subset$umap_cohort_cl %in% take_emt] <- "EMT.cells"

print("working on the combined subset Mesothelial+Progenitor+EMT")
...
mvp_sub <- ggplot(vst_out_sub$gene_attr, aes(log10(gmean), log10(residual_variance))) +
  ...
  geom_smooth()
ggsave(opt$outdir %&% opt$outName %&% ".subset_meso_prog_emt.mean_vs_variance_plot.png", mvp_sub, dpi = 300)
mvp2_sub <- ggplot(vst_out_sub$gene_attr, aes(log10(gmean), residual_variance)) +
  geom_point(alpha=0.3, shape=16) +
  geom_density_2d(size = 0.3) +
  geom_smooth()
ggsave(opt$outdir %&% opt$outName %&% ".subset_meso_prog_emt.mean_vs_variance_plot_2.png", mvp2_sub, dpi = 300)
...
saveRDS(sce_sub, opt$outdir %&% opt$outName %&% ".subset_meso_prog_emt.subtype.RDS")
