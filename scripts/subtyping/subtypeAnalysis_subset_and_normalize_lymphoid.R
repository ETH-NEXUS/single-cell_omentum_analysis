...
take_t = c(3,11,12,17,19,20,22,23)
take_plasma= c(21)
take_b = c(16)

all_subset <- combined[, combined$umap_cohort_cl %in% c(take_t, take_plasma, take_b)]
all_subset$subtype <- ""
all_subset$subtype[all_subset$umap_cohort_cl %in% take_t] <- "T.NK"
all_subset$subtype[all_subset$umap_cohort_cl %in% take_plasma] <- "Plasma"
all_subset$subtype[all_subset$umap_cohort_cl %in% take_b] <- "B.cells"

print("working on the combined subset lymphoid")
...
mvp_sub <- ggplot(vst_out_sub$gene_attr, aes(log10(gmean), log10(residual_variance))) +
  ...
  geom_smooth()
ggsave(opt$outdir %&% opt$outName %&% ".subset_lymphoid.mean_vs_variance_plot.png", mvp_sub, dpi = 300)
mvp2_sub <- ggplot(vst_out_sub$gene_attr, aes(log10(gmean), residual_variance)) +
  geom_point(alpha=0.3, shape=16) +
  geom_density_2d(size = 0.3) +
  geom_smooth()
ggsave(opt$outdir %&% opt$outName %&% ".subset_lymphoid.mean_vs_variance_plot_2.png", mvp2_sub, dpi = 300)
...
saveRDS(sce_sub, opt$outdir %&% opt$outName %&% ".subset_lymphoid.subtype.RDS")
