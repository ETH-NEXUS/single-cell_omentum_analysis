...
##### reference UMAPs: tissue region and clustering   #####
...
p_final_ref <- ggplot(colData_subset, aes(x = reducedDim(subset_combined)$umap1, y = reducedDim(subset_combined)$umap2, color = Tissue_Region)) +
  geom_point(size = 0.00001) +
  theme(aspect.ratio = 1) +
  xlab("UMAP 1") + ylab("UMAP 2") +
  theme(legend.position = "none")

p_legend_ref <- ggplot(colData_subset, aes(x = reducedDim(subset_combined)$umap1, y = reducedDim(subset_combined)$umap2, color = Tissue_Region)) +
  geom_point(size = 1) +
  xlab("UMAP 1") + ylab("UMAP 2") +
  guides(colour = guide_legend(override.aes = list(size = 2, shape = 15), nrow = 10)) +
...
sample_ref <- ggplot(colData_subset, aes(x = reducedDim(subset_combined)$umap1, y = reducedDim(subset_combined)$umap2, color = factor(umap_subtype_cl))) +
  geom_point(size = 0.00001) +
  theme(aspect.ratio = 1) +
  xlab("UMAP 1") + ylab("UMAP 2") +
  theme(legend.position = "none")

p_sample_legend <- ggplot(colData_subset, aes(x = reducedDim(subset_combined)$umap1, y = reducedDim(subset_combined)$umap2, color = factor(umap_subtype_cl))) +
  geom_point(size = 1) +
  xlab("UMAP 1") + ylab("UMAP 2") +
...
for (group in seq(length(gene.list.all))) {
  ...
  if (length(list_groups[[group_name]]) > 0) {
    ...
    for (gene in seq(length(list_groups[[group_name]]))) {
      ...
      merged_plots[["expr"]] <- ggplot(cell_attributes_gene, aes(x = reducedDim(subset_combined)$umap1, y = reducedDim(subset_combined)$umap2)) +
      ...
      merged_plots[["expr"]]
      # plot violin plot
      merged_plots[["violin"]] <- ggplot(cell_attributes_gene, aes(x = factor(umap_subtype_cl), y = normcounts)) +
