...
option_list = list(
  make_option("--rdsFile", type = "character", help = "Path to sce object file with input data."),
...
colData_df <- as.data.frame(colData(subset_combined))
umap_gex_df <- as.data.frame(reducedDim(subset_combined, "umap_gex"))
...
###   Join Results
md <- dplyr::left_join(colData_df, gsva_table, by = "barcodes")
